...
function ApprovalDialog({
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      enqueueSnackbar("✅ Order approved — confirmation email sent", {
        variant: "success",
      });
      onApproved();
      onClose();
    } catch (e: any) {
      enqueueSnackbar(e?.response?.data?.error ?? "Approval failed", {
        variant: "error",
      });
    } finally {
      setLoading(false);
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Typography sx={{ fontSize: "0.75rem", color: "text.secondary" }}>
              {order.customer_name} · {formatINR(order.total_amount)} ·{" "}
                    ...
                    <Grid container spacing={1}>
                      {[
                        ...
                        [
                          "Paid at",
                          order.paid_at
                            ? dayjs(order.paid_at).format("DD MMM HH:mm")
                            : "—",
                        ],
                        ["Status", order.status],
                      ].map(([l, v]) => (
          ...
          {step === 2 && (
            ...
            >
              ...
              >
                ...
                On approval, <strong>{order.customer_email}</strong> receives
                secure download links. Links expire in 30 days.
              </Alert>
              ...
              >
                ...
                <Box sx={{ p: 2.5, background: alpha("#111827", 0.5) }}>
                  ...
                  >
                    Hi {order.customer_name?.split(" ")[0]}, your order has been
                    approved. Here are your secure download links:
                  </Typography>
                  {(order.items ?? []).map((item) => (
                    <Box
                      key={item.id}
...
export default function OrdersPage() {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      enqueueSnackbar("Order rejected — customer notified by email", {
        variant: "info",
      });
      await mutate();
      setRejectOrder(null);
      ...
      enqueueSnackbar(e?.response?.data?.error ?? "Rejection failed", {
        variant: "error",
      });
    } finally {
  ...
  const columns: Column<Order>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <Box>
          <Typography
            sx={{ fontWeight: 700, color: "#4CAF82", fontSize: "0.875rem" }}
          >
          ...
          <Tooltip title="View details">
            <IconButton
              ...
              onClick={async () => {
                ...
                try {
                  const res = await api.get<OrderDetailResponse>(
                    `/admin/orders/${row.id}`,
                  );
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <Grid container spacing={2}>
                {[
                  ...
                  [
                    "Downloads",
                    selectedOrder.downloads_enabled ? "Enabled" : "Disabled",
                  ],
                ].map(([l, v]) => (
                  <Grid size={{ xs: 6 }} key={String(l)}>
                    <Typography
                ...
                {(selectedOrder.items ?? []).map((item) => (
                  ...
                  >
                    <Box>
                      ...
                      >
                        v{item.template_version} · qty {item.quantity} ·{" "}
                        {item.file_format ?? "—"}
                      </Typography>
                    </Box>
                ...
                selectedOrder.approved_at && (
                  ...
                  >
                    ...
                    {selectedOrder.downloads_expires_at && (
                      ...
                      >
                        Downloads expire:{" "}
                        {dayjs(selectedOrder.downloads_expires_at).format(
                          "MMM D, YYYY",
                        )}
                      </Typography>
                    )}
      ...
      >
        ...
        <DialogContent>
          <Alert
            ...
            sx={{ mb: 2.5, borderRadius: 2, fontSize: "0.82rem" }}
          >
            Customer will be notified by email. Provide a clear, professional
            reason.
          </Alert>
          <Typography

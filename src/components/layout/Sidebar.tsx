...
import {
  ...
  Typography, Divider, Avatar, Chip, Drawer, useMediaQuery, useTheme,
} from "@mui/material";
import {
  DashboardRounded, PeopleRounded, ShoppingCartRounded, ArticleRounded,
  AssignmentRounded, ContactMailRounded, EmailRounded, LayersRounded,
  CalculateRounded, LogoutRounded, CategoryRounded, AccountCircleRounded,
  SettingsRounded,
...
const ROLE_LABELS: Record<string, string> = {
  ...
  content_manager: "Content Mgr", order_manager: "Order Mgr", support_staff: "Support",
};
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Categories",  icon: <CategoryRounded />,  href: "/template-categories" },
      { label: "Calculators", icon: <CalculateRounded />, href: "/calculators" },
    ],
  },
  {
    title: "Account",
...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ px: 2.5, pt: 2.75, pb: 2 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
          <Box sx={{
            ...
            <Typography sx={{ fontWeight: 800, color: "#091422", fontSize: "1.1rem", fontFamily: '"DM Serif Display", serif', lineHeight: 1 }}>M</Typography>
          </Box>
          <Box>
            <Typography sx={{ fontWeight: 700, fontSize: "0.9rem", color: "#F0EDE8", letterSpacing: "0.01em" }}>Merraki</Typography>
            <Typography sx={{ fontSize: "0.58rem", color: alpha(GOLD, 0.55), letterSpacing: "0.12em", textTransform: "uppercase" }}>Admin Console</Typography>
          </Box>
        </Box>
      </Box>
      ...
      {/* Navigation */}
      <Box sx={{ flex: 1, overflow: "auto", px: 1.5, py: 1.75, "&::-webkit-scrollbar": { width: 0 } }}>
                ...
                return (
                  <ListItem key={item.label} disablePadding sx={{ mb: 0.2 }}>
                    <Link href={item.href} style={{ textDecoration: "none", width: "100%" }} onClick={onClose}>
                      <ListItemButton sx={{
                        borderRadius: 1.75, px: 1.25, py: 0.75, position: "relative",
                        background: active ? alpha(GOLD, 0.09) : "transparent",
                        color: active ? GOLD : "rgba(240,237,232,0.45)",
                        transition: "all 0.15s",
                        "&:hover": { background: alpha(GOLD, 0.06), color: "rgba(240,237,232,0.8)" },
                      }}>
                        {active && (
                          <Box sx={{
                            position: "absolute", left: 0, top: "50%", transform: "translateY(-50%)",
                            width: 3, height: 18, borderRadius: "0 3px 3px 0",
                            background: GOLD, boxShadow: `0 0 10px ${alpha(GOLD, 0.6)}`,
                          }} />
                        )}
                        <ListItemIcon sx={{ color: "inherit", minWidth: 32, "& svg": { fontSize: 18 } }}>
                          {item.icon}
                        </ListItemIcon>
                        <ListItemText primary={item.label} primaryTypographyProps={{ fontSize: "0.84rem", fontWeight: active ? 600 : 400 }} />
                      </ListItemButton>
                    </Link>
                  </ListItem>
                );
      ...
      <Box sx={{ p: 1.5, gap: 0.25, display: "flex", flexDirection: "column" }}>
          ...
          <Box sx={{
            ...
            <Box sx={{ position: "relative", flexShrink: 0 }}>
              <Avatar sx={{
                ...
                color: roleColor, border: `1.5px solid ${alpha(roleColor, 0.3)}`,
              }}>
                {initials}
              </Avatar>
              {/* Online dot */}
              <Box sx={{
                ...
                background: "#4CAF82", border: "1.5px solid #091422",
                boxShadow: `0 0 6px ${alpha("#4CAF82", 0.8)}`,
              }} />
            </Box>
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography sx={{ fontSize: "0.76rem", fontWeight: 600, color: "#F0EDE8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.4 }}>
                {admin?.email ?? "Loading…"}
              </Typography>
              <Chip label={roleLabel} size="small" sx={{
                height: 14, fontSize: "0.54rem", fontWeight: 700, mt: 0.2,
                background: alpha(roleColor, 0.1), color: alpha(roleColor, 0.85),
        ...
        <ListItemButton onClick={handleLogout} sx={{
          borderRadius: 2, px: 1.25, py: 0.8,
          color: "rgba(240,237,232,0.28)", transition: "all 0.15s",
          "&:hover": { background: alpha("#E05C5C", 0.07), color: "#E05C5C" },
        }}>
          <ListItemIcon sx={{ color: "inherit", minWidth: 32, "& svg": { fontSize: 17 } }}>
            <LogoutRounded />
          </ListItemIcon>
          <ListItemText primary="Sign Out" primaryTypographyProps={{ fontSize: "0.84rem" }} />
        </ListItemButton>
      </Box>
    </Box>
  );
...
export default function Sidebar({ mobileOpen, onMobileClose }: { mobileOpen: boolean; onMobileClose: () => void }) {
  ...
  if (isMobile) {
    return (
      <Drawer open={mobileOpen} onClose={onMobileClose} ModalProps={{ keepMounted: true }}
        sx={{ "& .MuiDrawer-paper": { width: SIDEBAR_W, border: "none", background: "transparent" } }}
      >
  ...
  return (
    <Box sx={{ width: SIDEBAR_W, flexShrink: 0, display: { xs: "none", lg: "block" } }}>
